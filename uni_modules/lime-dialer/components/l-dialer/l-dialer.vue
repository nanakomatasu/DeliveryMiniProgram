<template>
	<view class="l-dialer" :style="getStyle">
		<view class="l-dialer__inner" :style="getDialStyle">
			...
			<view class="l-dialer__inner-wrap" :style="styleOpt.borderColor && (' border: 8rpx solid ' + styleOpt.borderColor)">
				<view class="l-dialer__inner-item" v-for="(item, index) in prizeList" :key="index" :style="[getRotateAngle(index)]">
					<view class="l-dialer__inner-content" :style="[getCorrectAngle(index)]">
						...
						<block v-else>
							<view class="l-dialer__inner-name" :style="[{fontSize: styleOpt.fontSize, color: styleOpt.color, fontWeight: styleOpt.fontWeight}]">{{ item.name }}</view>
		...
		<view class="l-dialer__pointer" :style="pointerStyle" >
			...
			<image
				...
				@tap="$emit('click')"
			/>
			<view class="surplus-draw">{{ drawNum }}</view>
		</view>
	</view>
</template>
<script>
