			...
			<template v-for="(child, i) in dataList" >
				<scroll-view class="list" :key="i" v-if="i==selectedIndex" :scroll-y="true">
					<view class="item" :class="{'is-disabled': !!item.disable}" v-for="(item, j) in child" :key="j"
						...
						<text class="  f-28 c-88">{{item[map.text]}}
						
							<view class="" v-if="i==2 && selectedIndex==2" @tap.stop="smip" v-for="(itemx,indexm) in child[j].childrenx">
								<text class="item-text item-text-overflow m-t-12" v-if="item[map.text]==itemx.area">{{itemx.school_name}}</text>
							</view>
							
						</text>
						<view class="check" v-if="selected.length > i && item[map.value] == selected[i].value">
						</view>
					</view>
				</scroll-view>
			</template>
