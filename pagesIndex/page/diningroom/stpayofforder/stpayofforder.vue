	...
	<view v-if="detail">
		<view class="price dis-flex flex-y-center flex-x-center flex-dir-column">
			...
			<view class="f-28 col-gray-88 dis-flex flex-y-center">支付剩余时间<u-count-down
					:time="getRemainTime(detail.createdAt)" format="HH:mm:ss" @finish="finish"></u-count-down>
			</view>
			...
			finish() {
				...
				uni.showModal({
					...
					success(res) {
						if (res.confirm) {
							uni.redirectTo({
								url: "/pagesIndex/page/diningroom/canteenorder/canteenorder"
							})
						}
					}
				})
			...
			async getOrderPay() {
				...
				try {
					const {
						data,
						status
					} = await that.$api.inStoreOrderPay({
						order_id: this.id,
				...
				WeixinJSBridge.invoke('getBrandWCPayRequest', {
					...
					function(res) {
						...
						if (res.err_msg == "get_brand_wcpay_request:ok") {
							...
							let url = '/pagesIndex/page/diningroom/stplaceansuccess/stplaceansuccess?orderId=' + that
								.id
							//支付完成调回订单页面
							uni.redirectTo({
								url: url
