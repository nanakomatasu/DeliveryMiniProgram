<template>
	<view>
		<!-- 热搜词-->
		<u-sticky>
			<view class="pl26 pr26  pt12 pb12  box-size bg-white-ff dis-flex flex-y-center">
					...
					<swiper-item v-for="(item, index) in menuArr" :key="index">
						<view class="dis-flex flex-y-center flex-wrap">
							<view class="dis-flex flex-dir-column flex-x-center mb40  flex-y-center w-20"
								@tap="toLink('/pagesIndex/page/localLife/list/list?id='+items.id+'&name='+items.name)"
		...
		methods: {
			...
			async getStorecateIndex() {
				...
				const {
					...
					pid: this.id, //外卖固定为1，周边固定为2，优选固定为3
					limit: 100,
				})
				if (status == 200) {
			...
			async getStorelabelIndex() {
				...
				const {
					...
					cate_id: this.id, //外卖固定为1，周边固定为2，优选固定为3
					limit: 100,
				})
				if (status == 200) {
			...
			async getMerchantLists(isPage) {
				...
				const {
					...
					name: this.keyword, //
					category: this.id, //商家分类id 外卖固定为1，周边固定为2，优选固定为3
					page: this.page,
					sort: this.sortId,
