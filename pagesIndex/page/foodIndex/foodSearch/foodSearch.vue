			...
			<view class="">
				...
				<view class=" flex-wrap padd-l-12">
					<view class=" bor-r-26 m-l-20 m-t-20 bgc-f7 f-24" v-for="(item,index) in historyList" :key="index" >
						<u-tag :text="item" shape="circle" 
						bgColor="#e8e8e8"
						borderColor="#f7f7f7"
						color="#929399"  size="mini" :show="close1" @click="choseKeyword(item)">
						</u-tag>
					</view>
				</view>
			...
			<view class="m-t-40">
				...
				<view class=" flex-wrap padd-l-12">
					<view class="bor-r-26 m-l-20 m-t-20 bgc-f7 f-24" v-for="(item,index) in hotList" :key="index" >
						...
						bgColor="#e8e8e8" 
						borderColor="#f7f7f7"
						color="#929399" shape="circle" size="mini" :show="close1" @click="choseKeyword(item)" >
						</u-tag>
					</view>
				</view>
			</view>
				...
				<view v-for="(item1,index) in recommendList" :key="index" class=" c1 m-r-20">
					<view class="hot-box padd-12" :class="index==1 ? 'bgc-FDF1E6':' bgc-FBEAEB'">
						...
						<view class="padd-34 m-t-22 hot-box-main">
							<view class="flex-al-c m-b-40" v-for="(item2,index) in item1.keyword" :key="index" @click="choseKeyword(item2)">
	...
	export default {
		...
		methods: {
			...
			choseKeyword(keyword) {
				...
				uni.navigateTo({
					url: "foodSearchResult/foodSearchResult?keyword="+keyword
				})
			},
			//删除历史记录 
			async deleteHistory() {
