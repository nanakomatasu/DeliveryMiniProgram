			...
			async getOrderPay() {
				...
				try {
					const {
						data,
						status
					} = await that.$api.delicacyorderzf({
						order_id: this.id,
			...
			onBridgeReady(params) {
				...
				WeixinJSBridge.invoke('getBrandWCPayRequest', {
					...
					function(res) {
						//支付完成调回订单页面
						uni.redirectTo({
							// url: '/pagesIndex/page/foodIndex/sbuySuccess/sbuySuccess?qrcode='+JSON.stringify(this.detail.qrcode)
							url: '/pagesUser/page/orderList/orderList?id=3'
						})
